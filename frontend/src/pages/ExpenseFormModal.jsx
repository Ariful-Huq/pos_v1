      ...
      <form className="space-y-3" onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="block text-sm font-medium text-ink-700 mb-1">{t("expenses.category")}</span>
            {addingCategory ? (
              ...
              <div className="flex gap-1">
                ...
                <button type="button" onClick={() => setAddingCategory(true)} className="px-2 text-brand-700" title={t("expenses.category")}>
                  <Plus size={18} />
                </button>
              </div>
            )}
          ...
          <label className="block">
            <span className="block text-sm font-medium text-ink-700 mb-1">{t("expenses.amount")}</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="block text-sm font-medium text-ink-700 mb-1">{t("expenses.date")}</span>
            <input type="date" value={form.expense_date} onChange={(e) => set("expense_date", e.target.value)} required className="input" />
          </label>
          <label className="block">
            <span className="block text-sm font-medium text-ink-700 mb-1">{t("expenses.paymentMethod")}</span>
            <select value={form.payment_method} onChange={(e) => set("payment_method", e.target.value)} className="input">
              ...
              <option value="bank">{t("pos.methodBank")}</option>
              <option value="mobile_banking">{t("pos.methodMobileBanking")}</option>
            </select>
          </label>
        </div>

        <label className="block">
          <span className="block text-sm font-medium text-ink-700 mb-1">{t("expenses.description")}</span>
